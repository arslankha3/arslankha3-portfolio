
const RotatingGeometric = () => {
  return (
    <div
      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px]
      sm:w-[900px] sm:h-[900px] md:w-[1100px] md:h-[1100px] xl:w-[1200px] xl:h-[1200px] dark:hidden z-0"
    >
      <svg
        width="1224"
        height="1224"
        viewBox="-100 -100 2124 2124"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="w-full h-full rotating-geometrics"
      >
        <rect
          x="1574.33"
          y="1273.41"
          width="301.429"
          height="230"
          transform="rotate(-0.658359 1574.33 1273.41)"
          fill="#E699D9"
        ></rect>
        <rect
          x="12.5039"
          y="1061.34"
          width="301.429"
          height="230"
          rx="115"
          transform="rotate(-0.658359 12.5039 1061.34)"
          fill="#3CDD8C"
        ></rect>
        <rect
          x="821.238"
          y="14.1255"
          width="301.143"
          height="301.143"
          rx="45.7143"
          transform="rotate(-0.658359 821.238 14.1255)"
          fill="#3CDD8C"
        ></rect>
        <g>
          <path
            d="M695.056 1839.27C694.839 1820.33 710.012 1804.81 728.946 1804.59L1020.36 1801.24C1039.29 1801.02 1054.81 1816.2 1055.03 1835.13C1055.25 1854.07 1040.08 1869.59 1021.14 1869.81L729.734 1873.16C710.799 1873.37 695.274 1858.2 695.056 1839.27Z"
            fill="#E8E8FC"
          ></path>
          <path
            d="M693.874 1736.42C693.657 1717.48 708.83 1701.96 727.764 1701.74L1019.17 1698.39C1038.11 1698.17 1053.63 1713.35 1053.85 1732.28C1054.07 1751.21 1038.9 1766.74 1019.96 1766.96L728.552 1770.31C709.618 1770.52 694.092 1755.35 693.874 1736.42Z"
            fill="#E8E8FC"
          ></path>
          <path
            d="M696.238 1942.12C696.021 1923.18 711.193 1907.66 730.128 1907.44L1021.54 1904.09C1040.47 1903.87 1056 1919.05 1056.21 1937.98C1056.43 1956.92 1041.26 1972.44 1022.32 1972.66L730.916 1976.01C711.981 1976.23 696.456 1961.05 696.238 1942.12Z"
            fill="#E8E8FC"
          ></path>
        </g>
        <g>
          <path
            d="M1285.63 330.953C1285.41 312.019 1300.58 296.493 1319.52 296.276L1610.93 292.927C1629.86 292.71 1645.39 307.882 1645.6 326.817C1645.82 345.751 1630.65 361.276 1611.71 361.494L1320.3 364.843C1301.37 365.06 1285.84 349.887 1285.63 330.953Z"
            fill="#FFCCCC"
          ></path>
          <path
            d="M1284.44 228.103C1284.23 209.169 1299.4 193.643 1318.33 193.425L1609.74 190.077C1628.68 189.859 1644.2 205.032 1644.42 223.966C1644.64 242.901 1629.47 258.426 1610.53 258.644L1319.12 261.992C1300.19 262.21 1284.66 247.037 1284.44 228.103Z"
            fill="#FFCCCC"
          ></path>
          <path
            d="M1286.81 433.804C1286.59 414.869 1301.76 399.344 1320.7 399.126L1612.11 395.778C1631.04 395.56 1646.57 410.733 1646.78 429.667C1647 448.601 1631.83 464.127 1612.9 464.344L1321.49 467.693C1302.55 467.911 1287.03 452.738 1286.81 433.804Z"
            fill="#FFCCCC"
          ></path>
        </g>
        <rect
          x="168.176"
          y="1491.73"
          width="360"
          height="161.143"
          rx="22.8571"
          transform="rotate(-0.658359 168.176 1491.73)"
          fill="#E699D9"
        ></rect>
        <rect
          x="1652.47"
          y="924.635"
          width="360"
          height="161.143"
          rx="22.8571"
          transform="rotate(-0.658359 1652.47 924.635)"
          fill="#FFC435"
        ></rect>
        <rect
          x="30.1367"
          y="482.532"
          width="306.429"
          height="369.286"
          rx="57.1429"
          transform="rotate(-0.658359 30.1367 482.532)"
          fill="#FFC435"
        ></rect>
        <rect
          x="1294.24"
          y="1640.23"
          width="284.286"
          height="243.571"
          rx="70.2857"
          transform="rotate(-0.658359 1294.24 1640.23)"
          fill="#7575F0"
        ></rect>
        <rect
          x="1615.23"
          y="605.755"
          width="284"
          height="106.286"
          rx="53.1429"
          transform="rotate(-0.658359 1615.23 605.755)"
          fill="#E699D9"
        ></rect>
        <rect
          x="371.98"
          y="267.876"
          width="284"
          height="106.286"
          rx="53.1429"
          transform="rotate(-0.658359 371.98 267.876)"
          fill="#E8E8FC"
        ></rect>
      </svg>
    </div>
  );
};

const RotatingGeometric2 = () => {
  return (
    <div
      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px]
      sm:w-[900px] sm:h-[900px] md:w-[1100px] md:h-[1100px] xl:w-[1200px] xl:h-[1200px] dark:hidden z-0"
    >
      <svg
        width="1224"
        height="1224"
        viewBox="-100 -100 2124 2124"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="w-full h-full rotating-geometrics"
      >
        {/* Curly Braces */}
        <text
          x="150"
          y="500"
          fontSize="280"
          fontWeight="bold"
          fill="#7575F0"
        >
          {"{"}
        </text>
        <text
          x="1700"
          y="500"
          fontSize="280"
          fontWeight="bold"
          fill="#7575F0"
        >
          {"}"}
        </text>

        {/* Database cylinder */}
        <ellipse cx="1100" cy="200" rx="180" ry="50" fill="#FFC435" />
        <rect x="920" y="200" width="360" height="250" fill="#FFC435" />
        <ellipse cx="1100" cy="450" rx="180" ry="50" fill="#E699D9" />

        {/* Cloud (deployment / infra) */}
        <path
          d="M600 900c-40-60-140-60-180 0-60-10-110 30-110 90s50 100 110 90h290c60 0 110-40 110-90s-50-100-110-90c-20-40-70-60-110-60z"
          fill="#3CDD8C"
        />

        {/* Hexagon (system/microservice) */}
        <polygon
          points="1600,1400 1700,1470 1700,1590 1600,1660 1500,1590 1500,1470"
          fill="#FFCCCC"
        />

        {/* API Nodes (circle with connections) */}
        <circle cx="400" cy="1600" r="80" fill="#E8E8FC" />
        <circle cx="250" cy="1750" r="50" fill="#E699D9" />
        <circle cx="550" cy="1750" r="50" fill="#FFC435" />
        <line x1="400" y1="1600" x2="250" y2="1750" stroke="#999" strokeWidth="4" />
        <line x1="400" y1="1600" x2="550" y2="1750" stroke="#999" strokeWidth="4" />
      </svg>
    </div>
  );
};

export default RotatingGeometric;
